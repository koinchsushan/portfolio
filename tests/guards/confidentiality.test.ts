...
const FORBIDDEN_TOKEN_HASHES = new Set([
  ...
  // A hash for a publicly marketed third-party product name was deliberately
  // removed from this set: that name is public information (listed on its
  // own maker's public website), not a secret, so it is not a leak risk.
  // The rule governing it ("never claim it as the site owner's work") is an
  // accuracy rule, enforced by content review, not by this leak-scanning guard.
...
async function projectTextFiles(): Promise<string[]> {
  return globby(['**/*.{ts,tsx,js,jsx,css,md,json,html,svg,txt}'], {
    gitignore: true,
    ignore: ['tests/guards/confidentiality.test.ts', 'package-lock.json'],
  })
}
