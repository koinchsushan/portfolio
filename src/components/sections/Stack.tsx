...
export function Stack() {
  ...
  return (
    <section id="stack" aria-labelledby="stack-heading" className="border-b border-rule">
      <div className="mx-auto max-w-[1400px] px-6 py-14 sm:py-20">
        ...
        <div className="mt-10 border border-rule bg-surface">
          <div className="flex items-baseline justify-between gap-4 border-b border-rule px-4 py-3 sm:px-6">
            <span className="font-mono text-12 uppercase tracking-[0.14em] text-muted">Reference plate</span>
          ...
          <ul
            ...
            className="columns-1 [column-rule:1px_solid_var(--color-rule)] lg:columns-2"
          >
            {skillGroups.map((group, index) => {
              const chips = (
                <ul className="mt-3 flex flex-wrap gap-1.5">
                  {group.skills.map((skill) => (
                    <li
                      key={skill}
                      className="rounded-[var(--radius)] border border-rule bg-paper px-2 py-1 font-mono text-12 leading-none text-muted"
                    >
                      {skill}
                    </li>
                  ))}
              ...
              return (
                <li
                  key={group.label}
                  className="break-inside-avoid-column border-b border-rule px-4 py-4 sm:px-6 sm:py-5"
                >
                  <div className="flex items-baseline gap-3">
                    <span className="font-mono text-12 text-muted">{String(index + 1).padStart(2, '0')}</span>
                    <h3 className="font-subhead text-16 text-ink">{group.label}</h3>
                    <span className="ml-auto font-mono text-12 text-muted">{group.skills.length}</span>
                  </div>
